...
def generate_caption(image: torch.Tensor, image_encoder: Encoder, emb_layer: torch.nn.Embedding, image_decoder: Decoder, vocab: Vocab, device: torch.device):
...
if __name__ == '__main__':
    ...
    if LOAD_WEIGHTS:
        ...
        emb_layer.load_state_dict(torch.load(config.EMBEDDING_WEIGHT_FILE,map_location=torch.device('cpu')))
        image_encoder.load_state_dict(torch.load(config.ENCODER_WEIGHT_FILE,map_location=torch.device('cpu')))
        image_decoder.load_state_dict(torch.load(config.DECODER_WEIGHT_FILE,map_location=torch.device('cpu')))
